...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        operandseval = operands.map(lambda arg: scheme_eval(arg, env))
        return scheme_apply(self, operandseval, env)
        # END PROBLEM 5
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            selffncall = self.fn(*python_args)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        shorthandprocedure = LambdaProcedure(target.second, expressions.second, env)
        env.define(target.first, shorthandprocedure)
        return target.first
        # END PROBLEM 10
    else:
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 9
    lambdaform = LambdaProcedure(formals, expressions.second, env)
    return lambdaform
    # END PROBLEM 9
...
def do_and_form(expressions, env):
    ...
    while expressions is not nil:
        evalexpr1 = scheme_eval(expressions.first, env)
        if evalexpr1 is False:
            return False
        if expressions.second is nil:
            return evalexpr1
...
def do_or_form(expressions, env):
    ...
    while expressions is not nil:
        if expressions.second is nil:
            return scheme_eval(expressions.first, env, True)
        evalexpr = scheme_eval(expressions.first, env)
        if evalexpr is not False:
            return evalexpr
        expressions = expressions.second
    # END PROBLEM 13
...
def do_define_macro(expressions, env):
    ...
    # BEGIN Problem 21
    check_form(expressions, 2)
    macrotarget = expressions.first
    if scheme_symbolp(macrotarget):
        macrovalue = scheme_eval(expressions.second.first, env)
        env.define(macrotarget, macrovalue)
        return macrotarget
    elif isinstance(macrotarget, Pair) and scheme_symbolp(macrotarget.first):
        shorthandprocedure = MacroProcedure(macrotarget.second, expressions.second, env)
        env.define(macrotarget.first, shorthandprocedure)
        return macrotarget.first
    else:
        bad_target = macrotarget.first if isinstance(macrotarget, Pair) else macrotarget
        raise SchemeError('non-symbol: {0}'.format(bad_target))
    # END Problem 21
...
def do_mu_form(expressions, env):
    ...
    check_formals(formals)
    # BEGIN PROBLEM 16
    muform = MuProcedure(formals, expressions.second)
    return muform
    # END PROBLEM 16
